use super::{hash_slice, EmbedSuffix};
use crate::Node;

pub struct EmbedTrie {
...
impl EmbedTrie {
        ...
        K: AsRef<str>,
    {
        ...
        let emb_pos = self.get_value(node_idx) as usize;
        let emb_suf = EmbedSuffix::new(self.nodes[emb_pos]);

        let suffix: Vec<_> = chars.map(|c| c as u32).collect();
        if suffix.len() != emb_suf.len() {
            return None;
        }
        if hash_slice(&suffix) != emb_suf.hash_code() {
            return None;
        }
        Some(emb_suf.value())
    }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while self.text_pos < self.text.len() {
            ...
            if self.trie.is_leaf(self.node_idx) {
                let emb_pos = self.trie.get_value(self.node_idx) as usize;
                let emb_suf = EmbedSuffix::new(self.trie.nodes[emb_pos]);
                let text_suf = &self.text[self.text_pos..];
                let text_epos = self.text_pos + emb_suf.len();
                self.text_pos = self.text.len();
                if self.text.len() < text_epos {
                    return None;
                }
                if hash_slice(&text_suf[..emb_suf.len()]) != emb_suf.hash_code() {
                    return None;
                }
                return Some((emb_suf.value(), text_epos));
...
mod tests {
    ...
    #[test]
    fn test_common_prefix_search_en() {
        let keys = vec!["ab", "abc", "adaab", "bbc"];
        let trie = Builder::new()
            .set_suffix_thr(1)
            .from_keys(&keys)
